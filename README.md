...
week, season, `context` (jsonb, nullable). The site reads the `latest_board` view (latest run's rows per
...
### `context` on NFL edges + the `stattree` table (id=`latest`)

StatTree (thestattree.com) free daily sheets, fetched by the collector each
run. Their numbers are historical hit frequency vs the line - context, not a
projection; never blend them into the player-level model. On each matched NFL
edge, `context` carries: `line`/`odds` (sportsbook consensus - a third market
line next to FD and PP), `hit_rates` ({szn, l10, l5, l3} as [rate, "n/N"]),
`per_g`, `spread`, `gametotal`, `impliedtotal`, `proe`, opponent defense
ranks/rates, injury badges, depth-chart label, stadium/roof/rest. Coverage is
starters/rotation for 6 stats (pass_yds, pass_tds, rush_yds, rec_yds, rec,
any_td); absence of `context` means no StatTree row.

The `stattree` table row id=`latest` holds: `meta` (per-sheet generated_at +
